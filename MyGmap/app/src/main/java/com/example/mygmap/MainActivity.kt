...
class MainActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            ...
            4 -> {
                mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(37.65186, 127.01615),15f))
                mMap.addMarker(MarkerOptions().position(LatLng(37.65186, 127.01615)).title("덕성여자대학교"))
                return true
            }
            5 -> {
                mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(37.568256, 126.897240),15f))
                mMap.addMarker(MarkerOptions().position(LatLng(37.568256, 126.897240)).title("월드컵경기장"))
                return true
            }
            6 -> {
                mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(37.79630, 128.89679),15f))
                mMap.addMarker(MarkerOptions().position(LatLng(37.79630, 128.89679)).title("경포대"))
                return true
            }
        }
        return false
